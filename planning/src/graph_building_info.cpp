...
{
    if(filename.empty()){
        m_logger.printOut(LogLevel::ERROR, __FUNCTION__, "Invalid filename" );
        return false;
    }
    ...
    if(!file.is_open()){
        m_logger.printOut(LogLevel::ERROR, __FUNCTION__, "Unable to open file " + filename );
        return false;
    }
...
bool GraphBuildingInfoManager::loadEdgesInfo(const std::string &filename)
{
    ...
    try{
        ...
        if(!file.is_open()){
            m_logger.printOut(LogLevel::ERROR, __FUNCTION__, "Error reading graph from " + filename);
            return false;
        }
        ...
        while(std::getline(file, line)){
            ...
            if(info.size() < expected){
                m_logger.printOut(LogLevel::ERROR, __FUNCTION__, "Error reading graph from " + filename + ": not enough data in line");
                return false;
            }
        ...
        m_logger.printOut(LogLevel::ERROR, __FUNCTION__, "Error reading graph from " + filename  + " - Exception cought: " + e.what());
        return false;
    }
}
