...
{
    m_defCalculator.logger().setParent(loggerPtr());
}
...
Unit EMC_MassGrid::getUnits()
{
    ...
    if(units == Unit::UNIT_CUSTOM){
        ...
        //logger().printOut(LogLevel::WARNING, __FUNCTION__, "Mass map units are set to UNIT_CUSTOM. Seting working units to UNIT_TONS_PER_HECTARE");
    }
    else if( !isMassUnit(units) && !isMassPerAreaUnit(units) ){
        units = Unit::UNIT_TONS_PER_HECTARE;
        logger().printOut(LogLevel::ERROR, __FUNCTION__, "Invalid mass map units. Seting working units to UNIT_TONS_PER_HECTARE");
    }
    return units;
}
        ...
        if (!errorTmp){
            ...
            try{
                ...
                if(map->getUnits() != Unit::UNIT_CUSTOM)
                    logger().printOut(LogLevel::WARNING, __FUNCTION__, "Error converting mass/area map units to t/ha : " + std::string( e.what() ) + ". Values read as t/ha." );
            }
            return massPerArea;
        }

        logger().printOut(LogLevel::WARNING, __FUNCTION__, "Error obtaining the massPerArea from maps. Using default (average) mass/area." );
    }
