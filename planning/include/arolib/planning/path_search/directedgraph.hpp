...
enum EdgeType{
    ...
    CROSS,/**< Cross edges. Edges connecting vertices of two different infield tracks */
    CROSS_HL,/**< Cross edges. Edges connecting vertices of two different headland tracks */
...
public:
    ...
    // copy assignment
    //virtual Graph& operator=(const Graph& other);Class to plan overload activities
    ...
    /**
     * @brief Has the subfield partial headlands?
     * @return The subfield has partial headlands
     */
    bool hasPartialHeadlands() const;
    ...
    /**
     * @brief Get the ap holding the vertices of each track (by track id)
     * @return Map holding the vertices of each track (by track id) >
     */
    const std::map<int, std::vector<vertex_t>>& tracks_vertices_map() const;

    /**
     * @brief Get the Ids of the infield extrema tracks
     * @return Ids of the infield extrema tracks
     */
    const std::set<int>& extremaTrackIds_IF() const;

    /**
     * @brief Get the map holding the ids of the tracks adjacent to each track (for inner-field)
     * @return Map holding the ids of the tracks adjacent to each track
     */
    const std::map<int, std::set<int>>& adjacentTrackIds_IF() const;

    /**
     * @brief Get the map holding the ids of the tracks adjacent to each track (for headland)
     * @return Map holding the ids of the tracks adjacent to each track
     */
    const std::map<int, std::set<int>>& adjacentTrackIds_HL() const;

    /**
     * @brief Get the set holding the ids of the outermost HL tracks
     * @return Set holding the ids of the outermost HL tracks
     */
    const std::set<int>& outermostTrackIds_HL() const;
    ...
    /**
     * @brief Get the vertices over the boundary
     * @return Vertices over the boundary
     */
    std::set<vertex_t>& boundary_vts();

    /**
     * @brief Get the vertices over the boundary
     * @return Vertices over the boundary
     */
    const std::set<vertex_t> &boundary_vts() const;
    ...
    /**
     * @brief Get the closest vertex inside a circle with a given center and radius
     * @param center Center point of the circle
     * @param radius radius of the circle
     ...
     */
    bool getClosestVertexInRadius(const Point &center, float radius, vertex_t& vt_out, const VertexFilterFct &filter = NoVertexFilterFct) const;
    ...
    /**
     * @brief Regenerates the verticesLocationMap based on the current vertices in the graph
     */
    void regenerateVerticesLocationMap();
   ...
   struct GraphMetaData{
       bool hasPartialHeadlands = false; /**< The subfield had partial headlands >*/
       ...
       std::map<int, std::vector<vertex_t>> tracks_vertices_map; /**< Map holding the vertices of each track (by track id) >*/
       std::set<int> extremaTrackIds_IF; /**< Ids of the infield extrema tracks >*/
       std::map<int, std::set<int>> adjacentTrackIds_IF; /**< Map holding the ids of the tracks adjacent to each track (IF) >*/
       std::map<int, std::set<int>> adjacentTrackIds_HL; /**< Map holding the ids of the tracks adjacent to each track (HL) >*/
       std::set<int> outermostTrackIds_HL; /**< Set holding the ids of the outermost HL tracks >*/
       ...
       std::set<vertex_t> boundary_vts; /**< Vertices over the boundary >*/
    ...
    * @return Number of vertices in the graph
    */
   const GraphMetaData& get_meta() const;
...
protected:
     ...
     * @brief Get the (meta) data of a graph
     */
    inline static const Graph::GraphMetaData& getGraphData(const Graph& graph){ return graph.m_meta; }
};
