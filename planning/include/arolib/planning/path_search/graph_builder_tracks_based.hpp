...
public:
     ...
     */
    AroResp addBaseRouteInformationToGraph(Graph & graph,
                                           const std::vector<Machine> &machines,
                                           const std::vector<Route> &baseWorkingRoutes,
...
private:
    ...
    struct BuilderWorkspace{
        ...
        std::set<size_t> connectingHLsIndexes; /**< Set containing the indexes of the connecting headlands */
     ...
     */
    void interconnectHeadlandTracks(BuilderWorkspace & ws, const Subfield &sf) const;

    /**
     * @brief Add edges connecting the vertices corresponding to connecting partial headlands
     * @param [in/out] ws Workspace
     * @param sf Subfield
     */
    void connectHeadlands(BuilderWorkspace & ws, const Subfield &sf) const;
     ...
     * @param [in/out] graph graph
     * @param baseWorkingRoutes Routes
     * @param [in/out] ws Workspace (disregarded if null). If set, graph must correspond to ws->graph
     */
    void addBaseRouteInformation(Graph & graph, const std::vector<Route> &baseWorkingRoutes, BuilderWorkspace *ws) const;
     ...
     * @param baseWorkingRoutes Routes
     */
    void addVisitPeriods(Graph & graph, const std::map<MachineId_t, Machine> &machines, const std::vector<Route> &baseWorkingRoutes) const;
     ...
     */
    void addAndConnectFirstRoutePoint(Graph& graph, BuilderWorkspace * ws, const RoutePoint& rp, MachineId_t machine_id, const RoutePoint &rp_next, vertex_t* vt_nextRP) const;
     ...
     */
    bool findClosestValidVertex(Graph &graph,
                                const Point& pRef,
    ...
    bool connectToClosestValidVertex(BuilderWorkspace& ws,
                                     ...
                                     vertex_t* vt_closest = nullptr,
                                     vertex_property* vp_closest = nullptr) const;
