...
public:
    ...
    struct PlanParameters{
        ...
        std::map<MachineId_t, double> restrictedMachineIds_futureVisits = {};/**< Ids of the machines for which the future visits will be checked to avoid driving into potentially busy nodes */
    };
    ...
    enum ExpansionExclussionReason{
        IN_OPEN_LIST_AND_BETTER, /**< 0: The node is in the open list with a lower cost */
        IN_VISITED_LIST_AND_BETTER, /**< 1: The node is in the visited list with a lower cost */
        OK, /**< 2: no reason not to visit --> is visited */
        ...
        IN_EXCLUDE_SET, /**< 5: The node is in the given exclude set */
        IN_CLOSED_LIST, /**< 6: The node is in the closed list */
        GRAPH_LOCATION, /**< 7: The node is in the wrong graph location (e.g. the search is done to reach an OL point in the headland, but the node belongs to the inner-field search set) */
        SUCCESSOR_TIMESTAMP_OVER_MAX_TIME_VISIT, /**< 8: The time to reach this node is higher than the maximum allowed time to visit a node */
        CURRENT_VERTEX_GOT_BUSY /**< 9: Before the machine left the node, the node got occupied (i.e. the machine cannot stay that long in the current node) */
    };
    /**
...
private:
     ...
     * @param harvested_mass_start Bunker mass of the machine at the (start) moment of the search
     ...
     * @param goal_vertex Goal vertex
     * @param machineId Id of the machine for which we are planning
     * @param max_time_visit Maximum time(-stamp) allowed to visit a vertex during the search (used to know if the machine can use the vertex based on the vertex timestamp)
     * @param max_time_goal Maximum time(-stamp) in which the goal must be reached
     * @param exclude Set of vertices to be excluded from the search path
     * @param goal_location Location of the goal vertex (inner-field, headland).
     * @param edgeCostCalculator Edge Cost Calculator. Temporary: if = nullptr, uses internal functions
     * @param restrictedMachineIds Ids of the machines for which the max_time_visit applies
